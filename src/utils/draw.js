...
export function drawSkeleton(ctx, landmarks, video, isMirrored, isMobile) {
  ...
  // Hitung Skala (Logic Hybrid Cover/Contain)
  ...
  for (const hand of landmarks) {
    // Gambar Garis (Tulang)
    ctx.strokeStyle = "rgba(0, 255, 0, 0.8)"; // Warna Hijau Terang
    ctx.beginPath();
...
// --- FUNGSI LAMA: DRAW OVERLAY (Update Parameter) ---
// Tambahkan parameter 'shouldClear' agar tidak menghapus skeleton
export function drawOverlay(ctx, canvas, video, tracksRef, labels, isMirrored = false, isMobile = false, shouldClear = true) {
  ...
  // PENTING: Hanya clear jika diminta (biar skeleton gak ilang)
  if (shouldClear) {
  ...
  // ... (Sisa logika LERP dan Drawing Kotak SAMA PERSIS seperti sebelumnya) ...
